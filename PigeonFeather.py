...
import plugins.RandomGetWeatherPlugin
...
class PigeonFeather(QMainWindow):
    ...
    def setup(self):
        ...
        self.connect(self.configureDialog, SIGNAL("ConfigureDialogOk"), self.saveConfig)
        ...
        self.getWeatherThread = GetWeatherQThread(self.config.get("main", "woeid"))
        self.getWeatherThread.start()

        self.connect(
            self.getWeatherThread, SIGNAL("WeatherUpdate"), self.processWeather
        )
        self.connect(
            self.getWeatherThread, SIGNAL("WeatherReadError"), self.showErrorMessage
        )
...
"""
    ...
    def createMenu(self):
        ...
        menu = QMenu(self)
        menu.addAction(
            QIcon("images/22/sunny.png"), "&Show Weather Report", self.showWeatherDialog
        )
        menu.addAction(
            QIcon("images/22/configure.png"), "&Configure", self.showConfigureDialog
        )
        menu.addAction(QIcon("images/22/help.png"), "&About", self.showAboutDialog)
    ...
    def saveConfig(self, config):
        ...
        try:
            ...
            self.showErrorMessage("Could not save configuration settings" + "to disk")
    ...
    def showErrorMessage(self, message):
        ...
        self.trayIcon.showMessage(
            "Application Error", message, QSystemTrayIcon.Critical
        )
    ...
    def processWeather(self, weather):
        ...
        self.weatherDialog.labelLastUpdate.setText(fetched.strftime("%H:%M:%S"))
        self.weatherDialog.setWindowTitle(
            "Weather report for " + city + ", " + region + " " + country
        )
        ...
        self.weatherDialog.labelWindChill.setText(str(chill) + "°" + tempUnit)
        self.weatherDialog.labelWindSpeed.setText(str(windSpeed) + " " + speedUnit)
        self.weatherDialog.labelWindDirection.setText(directionTextual)
        self.weatherDialog.labelHumidity.setText(str(humidity) + "%")
        self.weatherDialog.labelVisibility.setText(str(visibility) + " " + distanceUnit)
        self.weatherDialog.labelPressure.setText(str(pressure) + " " + pressureUnit)
        self.weatherDialog.labelRising.setText(pressureTendancy)

        # Set the image
        self.weatherDialog.labelIcon.setPixmap(QPixmap("images/64/" + iconFileName))
